document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderDetailFromCard(card) {
    ...
    const oldPrice =
      card.querySelector(".product-old-price")?.textContent.trim() || "";
    const discount =
      card.querySelector(".product-discount")?.textContent.trim() || "";
    const rating =
      card.querySelector(".product-rating .rating-text")?.textContent.trim() ||
      card.querySelector(".rating-text")?.textContent.trim() ||
      "";
    ...
    try {
      ...
      productObj = products.find(
        (p) =>
          (p.tensanpham && p.tensanpham.trim() === name) ||
          (p.hinhanh && img && img.includes(p.hinhanh))
      );
      if (productObj) {
    ...
    const desc =
      ...
      "Mô tả sản phẩm chưa có. Liên hệ cửa hàng để biết thêm thông tin kỹ thuật và khuyến mãi.";
