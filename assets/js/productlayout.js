...
productLink.addEventListener("click", function (event) {
  ...
  let page = ` <h1>📦 Quản lý sản phẩm</h1>
  ...
  document.querySelector(".content-area").innerHTML = page;
  ...
  function renderTable(page = 1) {
    ...
    `;
      ...
      row
        .querySelector(".edit")
        .addEventListener("click", () => openEditForm(p));
      document
...
`;
  ...
  function filterProducts() {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const cat =
        rows[i].getElementsByTagName("td")[1]?.textContent.trim() || "";
